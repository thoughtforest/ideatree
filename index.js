...
let recentActivities = 500; // affects the hash rate.
let activeMembers = 100; // affects the hash rate.
...
const lifecycle = setInterval(() => {
    if (!walletTree.isReady) {
        console.log(`Waiting for initialization... ${(new Date().getTime() - startTime)/1000}s elapsed.`);
        return;
    }
    ...
    if (walletTree.userWallets.length <= 0) {
        for(let i=0; i<3; i++) {
            walletTree.createWallet(`test${i}`);
        }
    }
    ...
    for (let i = 0; i < tryCounts; i++) {
        ...
        if (systemCode == guess) {
            ...
            walletTree.userWallets.forEach(w => {
                const stake = w.hashPower / recentActivities;
                const amount = blockReward * stake;
                walletTree.createTransaction(w.id, amount);
            });

            // console.log(`${tree.wallets.length} active wallets rewarded total ${blockReward} MDC!, System balance: ${systemTotal}`);
            // tree.log();
            break;
        }
    }
